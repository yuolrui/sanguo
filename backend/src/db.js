    ...
    try {
        ...
        if (fs.existsSync(imageDir)) {
            for (const ext of extensions) {
                // Check for filename matching general name (e.g., "曹操.jpg")
                const filename = `${name}.${ext}`;
                if (fs.existsSync(path.join(imageDir, filename))) {
                    // Return API path served by app.js static middleware
...
export async function initDB() {
  ...
  const rawGenerals = [
      ...
      { name: '周仓', stars: 3, str: 80, int: 40, ldr: 60, luck: 50, country: '蜀', description: '赤胆忠心，随关羽殉难。', keywords: 'Zhou Cang, beard' }, // Duplicate fix needed, renamed or keep as is
